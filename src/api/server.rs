...
//!   → `async fn ping(&self, host: &str, port: i32, ct: ()) -> Result<Option<ServerState>, Error>`
//! - `Task<ServerState?> PingAsync(ServerEntry entry, CancellationToken ct)`（重载 → 改名
//!   `ping_entry`，见日志重命名决策）
//!   → `async fn ping_entry(&self, entry: &ServerEntry, ct: ()) -> Result<Option<ServerState>, Error>`
...
//!   → `async fn discover_lan(&self, ct: ()) -> Result<mpsc::Receiver<LanServerEntry>, Error>`
...
//!   占位参数 `ct: ()`（候选：tokio_util::sync::CancellationToken，待依赖批次定案）。
