//! 资源包扫描 + pack.mcmeta 解析（B10，对应源：Services/Expansion/Local/Resourcepacks.cs）
//!
//! 源类 `Resourcepack : LocalResourceBase`（构造函数注入
//! `HttpClient http, string gameDirectory, string version, bool versionSegmented, string apiKey`）。
//! 实现 `ResourcepackManager::get_resource_pack_list`（api/local.rs）：
//! 1. 扫描资源包目录（zip 文件 + 含 pack.mcmeta 的子目录），版本分段目录为
//!    `{gameDirectory}/versions/{version}/resourcepacks`，否则 `{gameDirectory}/resourcepacks`
...
//! 5. 目录形式的资源包：先在内存中把目录重新压缩为 ZIP 再计算哈希（源 ComputeHashesForFolder）
...
use crate::api::local::ResourcepackManager;
use crate::error::Error;
use crate::models::expansion::curseforge::FingerprintsFilesMeta;
use crate::models::expansion::local::ResourcePackInfo;
...
/// 资源包管理器实现（源：class `Resourcepack : LocalResourceBase`，Resourcepacks.cs）。
pub(crate) struct ResourcepackService {
...
impl ResourcepackService {
    /// 创建资源包管理器（源：构造函数 `Resourcepack(HttpClient, string, string, bool, string)`）
    pub(crate) fn new(
        http: reqwest::Client,
        game_root: String,
        version: String,
    ...
    fn resourcepack_directory(&self) -> PathBuf {
        if self.version_segmented {
            ...
            Path::new(&self.game_root).join("resourcepacks")
        }
    }

    /// 扫描资源包条目（源：GetResourcePackFiles）。
    /// 两遍枚举对齐源 `Directory.GetFiles(..., "*.zip")`（文件）+
    /// `Directory.GetDirectories(...)`（含 pack.mcmeta 的目录）；
    /// 目录不存在 → 空列表（源 `return [];`）；枚举失败 → 错误上抛（源异常传播）。
    fn get_resource_pack_files(&self) -> Result<Vec<PathBuf>, Error> {
        let resourcepack_directory = self.resourcepack_directory();
        if !resourcepack_directory.is_dir() {
            return Ok(Vec::new());
        }
        ...
        for entry in std::fs::read_dir(&resourcepack_directory).map_err(|e| Error::Params {
            ...
            let path = entry
                ...
                .path();
            // 源：`File.Exists(Path.Combine(dir, "pack.mcmeta"))` 才视为资源包目录
...
impl ResourcepackManager for ResourcepackService {
    ...
    async fn get_resource_pack_list(&self) -> Result<Vec<ResourcePackInfo>, Error> {
        ...
        for entry in &entries {
            ...
            pack_infos.push(ResourcePackInfo {
